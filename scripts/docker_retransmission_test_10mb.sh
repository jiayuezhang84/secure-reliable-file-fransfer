...
CONFIG_NAME="config_retransmission.json"
TIMEOUT_SECONDS=300

TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/srft_10mb.XXXXXX")"
...
echo "Checking retransmission evidence"
if ! grep -q "retransmitted" "$SERVER_LOG"; then
  echo "Retransmission was not observed in the server log"
  echo "Logs saved to $SUMMARY_LOG"
  exit 1
fi
...
echo "10 MB retransmission test passed"
echo "Logs saved to $SUMMARY_LOG"
